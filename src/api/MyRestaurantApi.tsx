...
export const useCreateMyRestaurant = () => {
  ...
  const createMyRestaurantRequest = async (restaurantFormData: FormData): Promise<Restaurant> => {
    ...
    if (!response.ok) {
      throw new Error("Failed ti create restaurant");
    }
    return response.json();
  };
...
export const useGetMyRestaurantOrders = () => {
  const { getAccessTokenSilently } = useAuth0();
  const getMyRestauratOrders = async (): Promise<Order[]> => {
    const accessToken = await getAccessTokenSilently();
  ...
  const { data: orders, isLoading } = useQuery("fetchMyRestaurantOrders", getMyRestauratOrders);
  return {
    orders,
    isLoading,
  };
...
export const useUpdateMyRestaurantOrders = () => {
  const { getAccessTokenSilently } = useAuth0();
  const updateMyRestaurantRequest = async (updateStatusOrderRequest: UpdateOrderStatusRequest) => {
    const accessToken = await getAccessTokenSilently();
  ...
  const { mutateAsync: updateRestaurantStatus, isLoading, isError, isSuccess, reset } = useMutation(updateMyRestaurantRequest);
  if (isSuccess) {
